...
class CityScapes(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # image = Image.fromarray(image.astype(np.uint8),mode='RGB')
        # label=cv2.cvtColor(np.asarray(label_copy), cv2.COLOR_RGB2GRAY)
...
class RandomScale(object):
    ...
    def __call__(self, sample):
        ...
        # print(self.scale_low[0])
        # temp_scale = self.scale_low[0] + (self.scale_high[1] - self.scale_low[0]) * random.random()
